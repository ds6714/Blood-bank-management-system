...
db = mysql.connector.connect(host ="localhost", user="root", password="root@15", database='db')
...
def Donate_dbase():
    ...
    sqlquery = "Select unit from BloodBank where blood_type='" + bgrp + "';"
    ...
    sqlquery = "Update BloodBank set unit='" + unit + "' where blood_type='" + bgrp + "';"
    print(sqlquery)
...
# function that alters the database and decreases the unit of the blood group
def Request_dbase():
    global bgrp
    global bunit
    unit = bunit.get()
...
def request(*args, **kwargs):
    ...
    submitbtn = Button(window, text="Submit", command=Request_dbase, bg="DodgerBlue2", fg="white",
                       font=('arial', 10))
    submitbtn.grid(row=8, columnspan=3)
